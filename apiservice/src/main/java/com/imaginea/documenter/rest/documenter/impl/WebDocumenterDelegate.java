...
public class WebDocumenterDelegate extends DataCreation {
	...
	private static final Logger LOGGER = Logger
			.getLogger(WebDocumenterDelegate.class);
	...
	public void createData() throws ClassNotFoundException {
		LOGGER.info("class paths:" + this.classPaths);
		...
		for (ClassResponseEntity classInfo : classesInfo) {
			pathJsonMap
					.put(classInfo.getResourcePath(), gson.toJson(classInfo));
		}
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Path Json Map Prepared Sucessfully, Total elements "
					+ pathJsonMap.size());
		}
	}
